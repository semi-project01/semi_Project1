...
def desc04():
    ...
    for year, color in zip([2018, 2019], ['darkorange', 'forestgreen']):
        ...
        ax.plot(monthly_mean_df.index.strftime('%b'), monthly_mean_df['평균 기온'], marker='o', color=color, label=f'{year}')
        ax.legend()
        ax.set_title('Monthly Average Temperatures (2018 and 2019)')
