...
def create_temper_sales_graph(data, year):
    ...
    # 1e8로 나누어서 표시
    temperature_sales['이용금액'] = temperature_sales['이용금액'] / 1e8
...
def desc02():
    ...
    st.dataframe(selected_df_2018.set_index('일자')[['평균 기온', '업종_분류', '이용금액']], width=800)

    st.write("2019년 데이터:")
    create_temper_sales_graph(selected_df_2019, 2019)
    st.pyplot()
    st.write("2019년 데이터 표")
    st.dataframe(selected_df_2019.set_index('일자')[['평균 기온', '업종_분류', '이용금액']], width=800)
